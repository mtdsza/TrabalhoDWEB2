...
@section('content')
    ...
    <form action="{{ route('orcamentos.update', $orcamento->id_orcamento) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="card mb-4">
                ...
                <div class="mb-3">
                    ...
                    <select id="id_paciente" name="id_paciente" class="form-select" required>
                        @foreach ($pacientes as $paciente)
                            <option value="{{ $paciente->id_paciente }}" @if($paciente->id_paciente == $orcamento->id_paciente) selected @endif>{{ $paciente->nome }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-3">
                    ...
                    <select id="id_profissional" name="id_profissional" class="form-select" required>
                        @foreach ($profissionais as $profissional)
                            <option value="{{ $profissional->id_profissional }}" @if($profissional->id_profissional == $orcamento->id_profissional) selected @endif>{{ $profissional->nome }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <table class="table">
                    ...
                    <tbody id="procedimentos-list">
                        @foreach($orcamento->itens as $item)
                            <tr>
                                <td>{{ $item->procedimento->nome }}<input type="hidden" name="procedimentos[{{ $item->id_procedimento }}][id_procedimento]" value="{{ $item->id_procedimento }}"></td>
                                <td><input type="number" step="0.01" name="procedimentos[{{ $item->id_procedimento }}][valor_unitario]" value="{{ $item->valor_unitario }}" class="form-control" required></td>
